...
const int N = 30001, M = 998244353, mod = 1000000007;
...
{
    int i = 0, j = 0;
    while(i<v2.size() and j<v3.size())
    {
        if(v2[i] < v3[j]) v1.push_back(v2[i++]);
        else v1.push_back(v3[j++]);
    }
    while(i<v2.size()) v1.push_back(v2[i++]);
    while(j<v3.size()) v1.push_back(v3[j++]);
}
...
int query(int i, int ss, int se, int l, int r, int k)
{
    if(ss>r or se<l) return 0;
    if(ss>=l and se<=r) return tree[i].end() - upper_bound(all(tree[i]),k);
    return query(2*i,ss,mid(ss,se),l,r,k) + query(2*i+1,mid(ss,se)+1,se,l,r,k);
}
